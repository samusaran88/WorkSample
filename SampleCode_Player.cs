...
public class SampleCode_Player : MiniGamePlayerBase
{
    ...
    Dictionary<ulong, SampleCode_Sheep> sheepHerdDictionary = new ();
        ...
        if (NetworkManager.Singleton.ServerTime.TimeAsFloat - lastBarkTime > barkCoolTime)
        {
            foreach (Collider c in Physics.OverlapSphere(transform.position, barkRadius))
            {
                if (c.CompareTag("Enemy"))
                {
                    ...
                    {
                        SampleCode_Sheep sheep = c.transform.parent.GetComponent<SampleCode_Sheep>();
    ...
    public override void OnPrePlaying()
    {
        ...
        if (IsOwner)
        {
            ...
            director = GameContext.GetDirector<SampleCode_Director>();
